 ...
 * : 서블릿의 생명주기를 관리 하는 역할 
 * 	(서블릿의 생명주기 내에서 특정 이벤트가 발생시 해당 콜백메서드를 호출)
 ...
 * 				요청의 HTTP메소드에 따라 doXXX계열 메소드를 호출
 * 
 * 	doGet 등의 doXXX계열 메소드
 * 			: 요청이 발생할때마다 반복 호출
 * 				HTTP 메소드에 따라 service에 의해 호출됨
 * 	destroy : 서블릿의 객체가 파괴된 직후에 호출되는 메소드
 * 
 *  WAS의 서블릿을 이용한 요청처리 단계
 *  1) 서블릿 매핑정보를 통해 현재 요청을 처리할 서블릿을 검색
 *  2) 해당 서블릿의 객체 생성 여부를 판단
 *  3) 현재 요청이 최초의 요청이라면 (객체생선전) 해당 서블릿의 객체를 생성 > init호출
 *  	singleton정책에 따라 이미 생성된 객체에 대해 skip
 *  4) Thread pooling 정책에 따라 쓰레드 할당 > service호출
 *  
 * 
 * 
 */
