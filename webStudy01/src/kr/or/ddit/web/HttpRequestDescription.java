...
/**
 * Http프로토콜의 request packaging 규칙
 * Request Line : URL, Http Method, Protocol/ver
 ...
 * 		getParameterValues(name) : 동일한 파라미턴명으로 여러개의 값이 전송
 * 		Enumeration b = a.getParameterNames()
 ...
 * 			server.xml -> URIEncoding : 특정 인코딩방식으로 고정
 * 						  useBodyEncodingForURI : POST방식에서 사용하는 request.setCharacterEncoding(encoding)를 GET에서 사용할수있게 해줌
 * 		POST : request.setCharacterEncoding(encoding) - 파라미터를 꺼내기 전에 설정 
 * 2) 요청 헤더의 확보
